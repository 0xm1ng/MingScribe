...
功能：滚动与真分页（一屏一页、可双页对开）、目录、全文搜索、四种颜色划线与批注、导出 Markdown（可直接进 Obsidian）、阅读进度记忆、**按书记忆排版**（字号 / 行距 / 页宽）、日间/夜间主题；PDF 另有独立阅读视图（翻页 / 跳页 / 缩放 / 适应页宽与整页）。
...
src/
  ...
  pdf.js            PDF 支持：纯逻辑（页码/缩放/中间格式）+ 动态 import 内置 pdf.js
  app.js            显示层：界面与交互
  style.css         样式
...
19. **PDF 只在桌面版可用，且只渲染不支持标注。** 原因见「PDF 支持」一节：pdf.js 是 ES Module，`file://` 下浏览器禁止加载模块。另外 PDF 的**划线 / 批注 / 全文搜索 / 文本层选区**这一版都没做——它们需要「页码 + 矩形坐标」这套与流式内容不兼容的锚点体系。扫描件（无文字层）能正常渲染图片，但将来做搜索时也搜不到内容。PDF 正文不进缓存，所以关掉再打开需要重新选一次文件（进度本身会保留）。
...
| PDF | 桌面版内置 pdf.js 渲染 | 固定版式按页栅格化，版面与原文件一致；**网页版不支持**（原因见下） |
...
4. **网页版看不了 PDF，这是刻意的**：pdf.js v4+ 只发布 ES Module，而浏览器在 `file://` 下禁止加载模块（CORS）。桌面版跑在 `http://tauri.localhost`，所以能用。网页版会给出明确提示而不是白屏——与本项目「多格式转换依赖本机 Calibre」是同一个先例。
5. **不做的事**：PDF 的划线 / 批注需要另一套「页码 + 矩形坐标」锚点，与流式内容的坐标系无法共享，本版本没做；也**没有**文本层选区与全文搜索。

## 下一步（按优先级）

1. PDF 的全文搜索与文本层选区（pdf.js 的 `getTextContent` 已经接好，只差 UI）
2. 划线的导入（现在只能导出 Markdown，不能导回来）
