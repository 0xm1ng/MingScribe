...
**书架** —— TXT 与 EPUB 都能进书架，下次点一下直接续读。封面配色由书名散列得到，但**不是**直接拿哈希当色相：那样必然有一部分书名落进土黄 / 橄榄黄绿那段「怎么调都显脏」的区间，所以改成在一组精选色相锚点里取（18 个锚点 × 5 档明度），任意书名都能拿到一对调过的渐变，且常见规模的书架基本不会撞色：
...
src/
  ...
  cover.js          书架封面配色：书名 → 一对精选渐变（纯函数，避开脏色相区）
  app.js            显示层：界面与交互
  style.css         样式
