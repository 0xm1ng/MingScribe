...
| 页宽 | 顶部 `宽−` / `宽+`（30 ~ 52 字宽共 6 档；双页对开下隐藏，每页宽由容器自动决定） |
| 日间 / 夜间 | 顶部「主题」按钮 |
...
1. 桌面端（Tauri）已搭好骨架并集成 Calibre 转换；下一步补齐图标 / 自动更新等打包细节，并在真实机器上用 `npm run convert:check` 验证转换链路
2. 划线的云同步与冲突合并（复杂度高，最后做）
3. PDF 阅读视图（独立子项目，见「PDF 支持路线图」；与流式阅读器的划线 / 进度体系隔离）
