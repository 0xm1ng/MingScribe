...
**两种阅读模式**：默认是连续的**滚动**模式；点顶部「滚动」按钮切到**分页**模式后，一屏就是一页，翻页是整页替换，底部的「上一章 / 下一章」会换成页码（如 `3 / 12`）。`Esc` 关闭面板、划线、搜索、批注、导出这些功能在两种模式下都完全一样。
...
1. 打包成桌面端（Tauri / Electron）——桌面端可直接持有真实文件路径，彻底免去重选文件
